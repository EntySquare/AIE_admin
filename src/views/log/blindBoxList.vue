  ...
  <div class="container">
    <a-card>
      <a-typography-title :heading="6">
        盲盒空投/回收日志列表</a-typography-title
      >
      <a-row :gutter="16">
        <a-col :span="5">
          <a-input v-model="condition" placeholder="输入盲盒名" />
        </a-col>
      ...
      <a-table :data="tableData" style="margin-top: 20px" :pagination="false">
        <template #columns>
          <a-table-column title="盲盒名" data-index="name"></a-table-column>
          <a-table-column title="盲盒图片" data-index="image">
...
<script setup lang="ts">
  ...
  const tableData = ref<BlindBoxLog[]>([]);
  const modalVisible = ref<boolean>(false);
  const modalTitle = ref<string>('查看用户');
  const rid = ref<number>(-1);
  const selectorBlindBox = ref<ItemMap[]>([]);
  ...
  const queryItemSelector = async () => {
    ...
    try {
      ...
      selectorBlindBox.value = res.data.blindBox_list ?? [];
  ...
  // 查询材料列表
  const queryBlindBoxListData = async (
    params: BlindBoxLogParam = {
      blindBox_id: blindBoxId.value,
    ...
    setLoading(true);
    try {
      const res = await fetchBlindBoxLogList(params);
      tableData.value = res.data.blind_box_log_list;
