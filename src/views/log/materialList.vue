  ...
  <div class="container">
    <a-card>
      <a-typography-title :heading="6">
        材料空投/回收日志列表</a-typography-title
      >
      <a-row :gutter="16">
        <a-col :span="5">
      ...
      <a-table :data="tableData" style="margin-top: 20px" :pagination="false">
        <template #columns>
          ...
          <a-table-column title="材料图片" data-index="image">
...
<script setup lang="ts">
  ...
  const queryMaterialListData = async (
    ...
    setLoading(true);
    try {
      const res = await fetchMaterialLogList(params);
      tableData.value = res.data.material_log_list;
