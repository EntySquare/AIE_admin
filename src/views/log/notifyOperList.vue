        ...
        <template #columns>
          ...
          <a-table-column title="请求参数">
            <template #cell="{ record }">
              <a-button @click="copyText(record.oper_param)">复制文本</a-button>
                </template>
...
<script setup lang="ts">
  ...
  import { fetchNotifyOperLogList, OperLogParam, NotifyOperLog } from '@/api/log';
  ...
  const tableData = ref<NotifyOperLog[]>([]);
  ...
  const queryAdminOperListData = async (
    ...
    setLoading(true);
    try {
      const res = await fetchNotifyOperLogList(params);
      tableData.value = res.data.notify_oper_log_list;
